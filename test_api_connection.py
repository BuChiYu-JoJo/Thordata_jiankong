...
def test_api_connection_urlencoded():
    """测试使用URL编码格式的API请求（当前脚本使用的格式）"""
    print("\n" + "=" * 60)
    print("测试1: URL编码格式请求（当前脚本使用的格式）")
    print("=" * 60)
...
def test_api_connection_json():
    """测试使用JSON格式的API请求（旧格式）"""
    print("\n" + "=" * 60)
    print("测试2: JSON格式请求（旧格式）")
    print("=" * 60)
...
def main():
    ...
    print("- URL编码格式是当前脚本使用的格式（推荐）")
    print("- JSON格式是旧的格式")
    print()
    
    # 测试两种格式
    result1 = test_api_connection_urlencoded()
    result2 = test_api_connection_json()
    
    print("\n" + "=" * 60)
    print("测试总结")
    print("=" * 60)
    print(f"URL编码格式测试: {'✓ 通过' if result1 else '✗ 失败'}")
    print(f"JSON格式测试: {'✓ 通过' if result2 else '✗ 失败'}")
    print()
    ...
    print("\n推荐使用：")
    print("  监控脚本使用的URL编码格式（Content-Type: application/x-www-form-urlencoded）")
    print("  优点：使用engine参数，更简洁、直接")
    print()
